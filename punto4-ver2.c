...
float costoProducto(Producto *puntProducto);
...
float costoProducto(Producto *puntProducto){
    return (puntProducto->cantidad*puntProducto->PrecioUnitario);
}
...
void mostrarPedidos(Cliente *puntCliente, int cantidad){
    ...
    for (int i = 0; i < cantidad; i++)
    {
        ...
        for (int j = 0; j < puntCliente->CantidadProductosAPedir; j++)
        {
            printf("Producto ID:%d ------> Cantidad:%d \n",puntCliente->Productos->ProductoID,puntCliente->Productos->cantidad);
            printf("Tipo producto:%s ------> Precio unitario:%.2f\n",puntCliente->Productos->TipoProducto,puntCliente->Productos->PrecioUnitario);
            printf("\n");
            costoTotal = costoTotal + costoProducto(puntCliente->Productos);
            puntCliente->Productos++;
        }
